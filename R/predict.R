#' Predictions from a fitted \code{devgam} object
#'
...
#' @export
#'
predict.devgam <- function(object, newdata, type="link", prob=NULL, se.fit=FALSE, marginal=TRUE, 
exi = FALSE, trace = 0, ...) {
...
if (got.newdata) {
  ...
  if (is.null(object$data)) 
    stop("Supply `devgam' with `removeData = FALSE' if not supplying `newdata'.")
...
for (j in seq_len(nprob)) {
  ...
  if (family == c("degpd")) {
    if (degpd_m %in% c(1, 3)) {
      ...
      if (degpd_m == 2) {
        pj <- degpd_iG(pj, pars[, 3], pars[, 4], pars[, 5])
      } else {
  ...
  if (family == c("zidegpd")) {
    if (zidegpd_m %in% c(1, 3)) {
      ...
      if (zidegpd_m == 2) {
        ...
        pj <- ifelse(pj > 0, pj,0 )
        pj <- zidegpd_iG(pj, pars[, 3], pars[, 4], pars[, 5])
      } else {
...
if (family %in% c("egpd", "degpd","custom")) 
