...
#'
#' Function \code{devgam} fits generalised additive extreme-value models. It allows 
...
#' @param control a list of lists of control parameters to pass to inner and outer optimisers; defaults to \code{devgam.control()}
#' @param removeData logical: should \code{data} be removed from \code{devgam} object? Defaults to \code{FALSE}
...
#' m_gev <- devgam(fmla_gev, fremantle, family = "gev")
#'
#' \donttest{
#' 
#' data(COprcp)
...
#' m_gpd <- devgam(fmla_gpd, data=COprcp_gpd, family="gpd")
#'
...
#' m_gev2 <- devgam(fmla_gev2, data=COprcp_gev, family="gev")
#' summary(m_gev2)
#' plot(m_gev2)
...
#' m_pp <- devgam(fmla_gev2, COprcp, family="pp", pp.args=pp_args)
#'
#' ## estimate 0.98 quantile using asymmetric Laplace distribution
#'
#' fmla_ald <- prcp ~ s(lon, lat, k=15) + s(elev, bs="cr")
#' m_ald <- devgam(fmla_ald, COprcp, family="ald", ald.args=list(tau=.98))
#'
#' }
#'
#' @seealso \link{predict.devgam}
#'
#' @return An object of class \code{devgam}
#' 
#' @export
#' 
devgam <- function(formula, data, family="gev", correctV=TRUE, rho0=0, 
...
egpd.args=list(),degpd.args=list(),zidegpd.args=list(), custom.fns=list(), sp = NULL, gamma = 1) {

## setup family
family.info <- .setup.family(family, pp.args, egpd.args,degpd.args, zidegpd.args,formula, custom.fns)
...
## add extra things that make an devgam object
...
NULL
fdevgam <- function(...) {
message("`fdevgam' will soon be deprecated: please migrate to `devgam'.")
devgam(...)
}

#' Extract Model Fitted Values
#'
#' @param object a fitted \code{devgam} object
#' @param ... not used
#'
#' @examples
#'
...
#' @export
#' 
fitted.devgam <- function(object, ...) {
predict(object)
}

#' Simulations from a fitted \code{devgam} object
#'
#' @param object a fitted \code{devgam} object
#' @param nsim an integer giving the number of simulations
#' @param seed an integer giving the seed for simulations
#' @param newdata a data frame
#' @param type a character string, as in \code{predict.devgam}; defaults to \code{"quantile"}
...
#' @param ... arguments to be passed to \code{predict.devgam}
#'
...
#' @export
#' 
simulate.devgam <- function(object, nsim=1e3, seed=NULL, newdata, 
...
if (type %in% c("link", "response")) {
  ...
  X <- predict.devgam(object, newdata, type="lpmatrix")
  nms <- names(X)
...
if (type == "quantile") {
  X <- simulate.devgam(object, nsim, seed, newdata, "response")
  out <- list()
...
#' Log-likelihood, AIC and BIC from a fitted \code{devgam} object
#'
#' @param object a fitted \code{devgam} object
#' @param ... not used
#'
...
#' Plot a fitted \code{devgam} object
#'
#' @param x a fitted \code{devgam} object
...
#' @export
#' 
plot.devgam <- function(x, onepage = TRUE, which = NULL, main, ask = !onepage, ...) {
x <- x[x$gotsmooth]
if (is.null(which)) {
...
#' Summary method for a fitted \code{devgam} object
#'
#' @param object a fitted \code{devgam} object
#' @param ... not used
#'
#' @details
#' 
#' The key part of summary.devgam is p-values for smooths.
...
#'
#' @return A \code{summary.devgam} object
#'
#' @examples
#'
#' data(fremantle)
...
#' @export
#' 
summary.devgam <- function(object, ...) {
if (!missing(...)) warning("extra arguments discarded")
out <- list()
out[[1]] <- .parametric.summary.devgam(object)
out[[2]] <- .smooth.summary.devgam(object)
class(out) <- "summary.devgam"
out
}

#' @param x a \code{summary.devgam} object
#'
#' @rdname summary.devgam
#' 
#' @export
#' 
print.summary.devgam <- function(x, ...) {
if (!missing(...)) warning("extra arguments discarded")
cat("\n")
...
#' Print a fitted \code{devgam} object
#'
#' @param x a fitted \code{devgam} object
#' @param ... not used
#'
#' @return The call of the \code{devgam} object
#'
#' @examples
#'
#' data(fremantle)
...
#' \code{ginv.devgam} mimics \link[MASS]{ginv} using Armadillo's pinv.
#'
#' @return A matrix
#' 
#' @references
...
#' @export
#' 
ginv.devgam <- function(x, tol=sqrt(.Machine$double.eps)) {
armaginv(x, tol)
}
