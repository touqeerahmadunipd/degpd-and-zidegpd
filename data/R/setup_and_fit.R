...
#
# .setup.formula
# --- takes devgam formula and returns something useful for fitting
#
# .predictable.gam
# --- takes a gam(..., fit=FALSE) object and adds stuff so that mgcv::predict.gam works
#
# .X.devgam
# --- gives design matrics for a devgam object
#
# .setup.data
# --- takes data and devgam argument and return something useful for fitting
#
# .setup.pp.data
...
.setup.family <- function(family, pp, egpd,degpd,zidegpd, formula, likfns) {
if (family == "gev") {
  lik.fns <- .gevfns
  npar <- 3
  nms <- c("mu", "lpsi", "xi")
  ...
  if (family == "degpd") {
    ...
    if (degpd$m == 1) {
      ...
      if (degpd$m == 2) {
        lik.fns <- .degpd2fns
        npar <- 5
        nms <- c("lsigma", "lxi", "lkappa1", "lkappa2", "logitp")
        nms2 <- c('logscale', 'logshape', 'logkappa1', 'logkappa2', 'logitp')
        attr(family, "type") <- 2
      } else {
        if (degpd$m == 3) {
          lik.fns <- .degpd3fns
    ...
    if (family == "zidegpd") {
      ...
      if (zidegpd$m == 1) {
        ...
        if (zidegpd$m == 2) {
          lik.fns <- .zidegpd2fns
          npar <- 6
          nms <- c("lsigma", "lxi", "lkappa1", "lkappa2", "logitp", "logitpi")
          nms2 <- c('logscale', 'logshape', 'logkappa1', 'logkappa2', 'logitp', 'logitpi')
          attr(family, "type") <- 2
        } else {
          if (zidegpd$m == 3) {
            lik.fns <- .zidegpd3fns
...
.X.devgam <- function(object, newdata) {
...
if (nrow(data) > maxdata) {
    ...
    data <- data[id,]
    if (trace >= 0)
      message("`data' truncated to `maxdata' rows. Re-supply `data' to, e.g., `predict.devgam'")
}
...
if (family == "weibull") {
  if (min(lik.data$y) <= 0) 
    stop(expression("Weibull distribution has support (0, \U221E) in devgam."))
}
if (family == "gpd") {
  if (min(lik.data$y) <= 0) 
    stop(expression("GPD has support (0, \U221E) in devgam."))
}
if (family == "exi") {
...
if (!compact) {
  if (nrow(data) > maxspline) {
    lik.data$X <- .X.devgam(gams, data)
  } else {
    lik.data$X <- .X.devgam(gams)
  }
  if (family %in% c("pp", "ppexi")) {
    ppX <- .X.devgam(gams, attr(data, "quad"))
  ...
  lik.data$X <- .X.devgam(gams, data[unq.id,])#lapply(gams, .X.devgam, newdata=data[unq.id,])
...
if (correctV)
  fit.reml$invHessian <- .solve_devgam(fit.reml$Hessian)
...
Vc <- VpVc$Vc
if (smooths) {
  # bug fixed in devgam_0.1.2 if any(diff(gotsmooth)) != 1
